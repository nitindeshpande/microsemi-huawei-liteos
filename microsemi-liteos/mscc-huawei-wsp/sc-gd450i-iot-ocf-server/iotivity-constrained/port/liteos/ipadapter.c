...
{/*
  UINT32 uwRet;
  uwRet = LOS_MuxPend(mutex, LOS_WAIT_FOREVER);
  if(uwRet != LOS_OK)
  {
    LOG("ERROR lock network event thread\n");
  }*/
}
...
//	struct sockaddr_storage receiver;
  struct sockaddr_in *addr = NULL;
	memcpy(&addr->sin_addr.s_addr, message->endpoint.addr.ipv4.address,
				 sizeof(addr->sin_addr.s_addr));
	addr->sin_family = AF_INET;
	addr->sin_port = htons(message->endpoint.addr.ipv4.port);
...
{
  memset(&mcast4, 0, sizeof(struct sockaddr_in));
  memset(&server4, 0, sizeof(struct sockaddr_in));
	
  struct sockaddr_in *m = &mcast4;
  m->sin_family = AF_INET;
  m->sin_port = htons(OCF_PORT_UNSECURED);
  m->sin_addr.s_addr = INADDR_ANY;

  struct sockaddr_in *l = &server4;
...
//  mcast4_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
  ...
  server4_sock = socket(AF_INET, SOCK_DGRAM, 0);
  mcast4_sock = socket(AF_INET, SOCK_DGRAM, 0);
  ...
  if (bind(server4_sock, (struct sockaddr *)&server4, sizeof(struct sockaddr)) == -1) {
  ...
  if (bind(mcast4_sock, (struct sockaddr *)&mcast4, sizeof(struct sockaddr)) == -1) {
...
//  }
	...
	/***** create message handle thread in liteos *****/
  UINT32 uwRet;
  TSK_INIT_PARAM_S stInitParam;

  stInitParam.pfnTaskEntry = (TSK_ENTRY_FUNC)network_event_thread;
  stInitParam.usTaskPrio = TSK_PRIOR_HI;
  stInitParam.pcName = "HIGH_NAME";
  stInitParam.uwStackSize = 10240;
  stInitParam.uwResved   = LOS_TASK_STATUS_DETACHED;
  uwRet = LOS_TaskCreate(&event_thread, &stInitParam);
  if (uwRet != LOS_OK)
  {
      LOG("event_thread create Failed!\r\n");
      return LOS_NOK;
  }
...
#endif /* OC_SECURITY */
  UINT32 uwRet;
	uwRet = LOS_TaskDelete(event_thread);
  if(uwRet != LOS_OK)
  {
      LOG("event_thread delete failed .\n");
  }
