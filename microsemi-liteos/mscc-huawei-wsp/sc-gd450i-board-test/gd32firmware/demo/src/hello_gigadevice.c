...
static void hello_task(void *arg)
{
    ...
    while(1){
        ...
        while(-1 != newfd){
            ...
            recvnum = recv(newfd, hello_buf, MAX_NAME_SIZE, 0); 
            if (recvnum <= 0){
                lwip_close(newfd);
                newfd = -1;
                break;
            }
            hello_gigadevice_recv(newfd, hello_buf, recvnum);
        }
...
void hello_gigadevice_init(void)
{
    ...
    //xTaskCreate(hello_task, "HELLO", DEFAULT_THREAD_STACKSIZE, NULL, HELLO_TASK_PRIO, NULL);
		thread_def.name = "HELLO";
		thread_def.stacksize = 800;//DEFAULT_THREAD_STACKSIZE;
		thread_def.tpriority = osPriorityNormal;
		thread_def.pthread = (os_pthread)hello_task;
